...
- Unique Receipts attached to contributions add [Reinforcement](/commons#reinforcement),
  increasing the weight of a contribution as it provides additional grounding and support.
...
flowchart TB
        ...
        subgraph DERIVED["Derived"]
            ...
            subgraph FIELD[Field]
                ...
                subgraph CHARGE[Charge]
                  CHA_E(Expressive Charge):::expressive
                  CHA_C(Committal Charge):::committal
                end
    ...
    subgraph REGISTERS["Classification Registers"]
        ...
        subgraph DOMAIN["Active Domain"]
            ...
            subgraph EMERGENCE["Emergence"]
                ...
                ACTIVITY("<strong>Activity</strong><br><small>What is new</small>"):::activity
                FRONTIER("<strong>Frontier</strong><br><small>What is forming</small>"):::frontier
                CANON("<strong>Canon</strong><br><small>What persists</small>"):::canon
            end

            subgraph SETTLEMENT["Settlement"]
                direction TB

                SIGNAL("<strong>Signal</strong><br><small>What changed</small>"):::signal
                STANCE("<strong>Stance</strong><br><small>Where we stand</small>"):::stance
                CONCORD("<strong>Concord</strong><br><small>Where we agree</small>"):::concord
            end
        end

        COMMIT[Commit]:::committal
    end

    POTENTIAL -.->|"T<small>dir</small> > 0<br>P1"| ACTIVITY
    POTENTIAL -.->|"T<small>dir</small> < 0<br>P1"| SIGNAL
    POTENTIAL ---> PC
    ...
    CHA_C commitCharge@<--> COMMIT
    classDef reverseOnly marker-end:none;
    class commitCharge reverseOnly;

    %% Negative (-) edges
    linkStyle 11,13,15,17 stroke:#3b5bdb,color:#3b5bdb

    %% Positive (+) edges
    linkStyle 12,14,16,18 stroke:#748ffc,color:#748ffc
```
...
graph TD
  ...
  subgraph CH1["Context Horizon 1 (Domain A)"]
    ...
    subgraph D1["<strong>Depth 1</strong> (Tension: -0.34)"]
      A("Question")
    end

    subgraph D2["<strong>Depth 2</strong> (Tension: -0.27)"]
      B("Observation")
      C("Observation")
    end

    subgraph D3["<strong>Depth 3</strong> (Tension: -0.21)"]
      D("Evidence")
      E("Hypothesis")
      F("Question")
    end

    subgraph D4["<strong>Depth 4</strong> (Tension: -0.09)"]
      G("Question")
      H("Evidence")
      I("Evidence")
      J("Hypothesis")
  ...
  subgraph CH2["Context Horizon 2 (Domain B)"]
    subgraph D5["<strong>Depth 1</strong> (Tension: +0.17)"]
      K("Question"):::expressive
    end
  end

  I -->|"Closure Boundary"| K

  class A,B,E,I,K committal
...
Take the active nodes of the current lineage within a Context Horizon,
i.e. the nodes that haven't yet been hard superceded,
and then identify the aggregate lean.
...
- The only Propositions available to be Superceded (Gated) from are the ones that
  align with the aggregate lean direction of the current lineage within the active
  Context Horizon.
...
flowchart TB
  ...
  class P1,P4,P5,P7,P8,P10,V1,V3,V6,V9 committal
...
Receipts provide additional grounding and support for contributions.
This gets factored into the above calculations as an additional weighting extension.

- Not all Receipt Types count towards Reinforcement,
  as some types (such as Cite) can be easily obtained.
- Only **unique** Receipts get counted across all Participants,
  in an attempt to reduce coordinated efforts.
- The system determines the uniqueness of a Receipt, such as comparing
  file hashes or identifying an accurate last updated timestamp from the source.
