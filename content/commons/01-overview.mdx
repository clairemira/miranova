...
With the main goal in mind of conserving energy and resources,
can we build a system that aims to use the _least_ amount of
tokens possible for any given model?
...
graph TD
  ...
  subgraph CH["Context Horizon"]
    ...
    subgraph D0["<strong>Depth 0</strong> (Tension: -0.34)"]
      A["Question"]
    end

    subgraph D1["<strong>Depth 1</strong> (Tension: -0.27)"]
      B["Observation"]
      C["Observation"]
    end

    subgraph D2["<strong>Depth 2</strong> (Tension: -0.21)"]
      D["Evidence"]
      E["Hypothesis"]
      F["Question"]
    end

    subgraph D3["<strong>Depth 3</strong> (Tension: -0.09)"]
      G["Question"]
      H["Evidence"]
      I["Evidence"]
      J["Hypothesis"]
    ...
    subgraph D4["<strong>Depth 4</strong> (Tension: +0.03)"]
      K["Closure"]
    end
...
The implementation details are intentionally left open for experimentation and may change over time.
...
from biology, as the intention is for each measurement to provide a feedback loop
that pulls the system back towards equilibrium, rather than letting it drift.
...
structures are selected, subsequent contributions will more likely affect self Correspondence.
...
How similar in **depth and cadence** are Committal Propositions between Domains,
and how strongly is commitment reinforced?

The indigo nodes below represent Committal Propositions, while the indigo edges represent
the number of Participant Cycles that occurred between, highlighting the path to maximum depth.
...
Committal Depth is based on the number of Committal Propositions that form the longest chain.
Using the diagram above as an example, even though there may be multiple Expressive nodes
in between, only the Committal nodes count towards depth.

If one Domain has a longer chain of Committal Propositions than the other, then depth is Variable.
...
Receipts (1PP) count as additional weighting extension
to reinforce the commitment represented at that depth.
The initial Receipt from the owning Participant does not count
towards reinforcement, as a Receipt is already a precondition
for a Committal Proposition.

The same weighting method as Chirality is used as a default option,
with a ratio of `3:1` Commits to Receipts. This weighting is intended to be
...
This allows the frequency of commitment to be made relative to participation,
regardless of how much real world time passes between commits.

Taking the number of Cycles between each Committal Proposition, the
[Coefficient of Variation (CV)](https://en.wikipedia.org/wiki/Coefficient_of_variation)
measures how regularly commitment occurs relative to each Domain's average interval.
This allows for temporal dispersion to be compared between each Participant,
even when they commit at very different frequencies.
...
The average between Depth and Cadence represents the final Closure value.
This is a convenient first option, but implementation details may change with time.
...
- The balancing effect for Closure naturally falls out from the cost of writing;
  Commitment that causes high Tension allocates Charge, while Tension reducing
  commitment reclaims Charge.
...
If a stable and balanced substrate lies somewhere in the middle,
then each projection represents a shift away from stability.
Neither takes precedence over the other, they are each
complementary ways of representing imbalance.

### Prominence

Prominence represents a rigid and over-ordered structure,
with high dominance pressure favouring a particular direction.
...
As each of the above properties require

## Bootstrapping

- The first Proposition must be Committal,
...
Another way of looking at it is, do you spin _with_ the current direction
or do you against it.
...
- The implementations provided are working examples, and will be intended to be
  configurable when creating a substrate, to allow for experimentation with ideas.
