...
I'm currently developing The Commons and writing the documentation as I code.
...
that aims to use the _least_ amount of tokens for any given model?
...
- As Propositions get Reconciled, only the new active Proposition get

---

### 3. Closure

How similar in **depth and cadence** are Propositions between Domains,
...
- Both Depth and Cadence must align the same sign direction for commitment to occur,
...
If a stable and balanced substrate sits somewhere in the middle,
...
The above Measurements, Projections and Tension shift sign direction
with [hysteresis](https://en.wikipedia.org/wiki/Hysteresis).

As directional state is a requirement, when each of the above observables
reaches a value of zero, the previous direction remains the active
directional state until the value changes to the other direction.

## Bootstrapping

The bootstrapping process demonstrates how a substrate gets created,
while simultaneously providing it with Charge.

- Before any Proposition exists, the empty substrate is perfectly balanced with
  a Tension of 0.
- Tension's sign direction is defaulted positive (+) towards the Emergence
  Domain, as the Settlement Domain requires existing Propositions to resolve.
- The very first Proposition must be Committal, as Expressive Propositions
  can only exist as variants from existing Propositions.
- Committal Propositions can only get written into Canon & Concord registers,
  therefore the first written register is Canon.
- The initial Proposition gets written, and with it begins the first Context Horizon.
  - Producing 100% Turbulence, as there is nothing in the other Domain to compare to.
  - Flips Tension's sign direction to (-1), the active Domain for this

## Charge

Charge is a shared and conserved resource. It's simply what remains with lack of Tension.
