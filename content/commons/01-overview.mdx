...
I'm currently developing The Commons and writing the below documentation as I code.
...
- Rather than mapping one model to a specific problem, can we
  identify a dynamic MECW per model as the problem space changes?
- And by model that includes humans also.
...
- The only Propositions available to be Superceded (Gated) from are the ones that
  align with the dominant lean direction of the current Context Horizon.
- Propositions are Reconciled during this Phase, which results in a new
  Proposition getting written with zero Attestations.
- This then causes the system to maintain balance between Alignment & Divergence,
  as the dominant lean direction is forced to collapse towards zero.
...
regardless of how much real world time passes between them.
